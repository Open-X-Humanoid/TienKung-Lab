...
from legged_lab.envs.tienkung_pro.run_cfg import TienKungProRunFlatEnvCfg
from legged_lab.envs.tienkung_pro.run_with_sensor_cfg import TienKungRunWithSensorFlatEnvCfg
from legged_lab.envs.tienkung_pro.walk_cfg import TienKungProWalkFlatEnvCfg
...
class TienKungProEnv(VecEnv):
    def __init__(
        self,
        cfg: (
            TienKungProRunFlatEnvCfg
            | TienKungProWalkFlatEnvCfg
            | TienKungWalkWithSensorFlatEnvCfg
        ...
        self.curriculum_terms = getattr(self.cfg, "curriculum_terms", {})
    ...
    def reset(self, env_ids):
        ...
        self._apply_curriculum_terms(env_ids)
    ...
    def _apply_curriculum_terms(self, env_ids):
        if len(env_ids) == 0 or not self.curriculum_terms:
            return

        for _, term_cfg in self.curriculum_terms.items():
            if getattr(term_cfg, "mode", "reset") != "reset":
                continue
            term_cfg.func(self, env_ids, **getattr(term_cfg, "params", {}))
    ...
    def check_reset(self):
        ...
        # Check for abnormal robot orientation using projected gravity
        projected_gravity = self.robot.data.projected_gravity_b
        reset_buf |= (torch.abs(projected_gravity[:, 0]) > 0.7) | (torch.abs(projected_gravity[:, 1]) > 0.7)
        reset_buf |= projected_gravity[:, 2] > -0.6  # lost upright orientation
